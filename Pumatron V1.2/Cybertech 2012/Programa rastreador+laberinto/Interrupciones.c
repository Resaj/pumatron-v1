...
// Interrupción para la lectura de los sensores de distancia
...
//		LCDPutStr(cadena, 100, 10, SMALL, WHITE, BLACK);
	...
	{
		if(SUPERESTADO == Rastreador)
		{
			leer_pared();

			for(i=19; i>0; i--)
				pared_izq[i] = pared_izq[i-1];
			pared_izq[0] = sensor_izq;

			for(i=19; i>0; i--)
				pared_der[i] = pared_der[i-1];
			pared_der[0] = sensor_der;

			promedio_izq = 0;
			for(i=0; i<20; i++)
				promedio_izq += pared_izq[i];
			promedio_izq = promedio_izq/20;

			promedio_der = 0;
			for(i=0; i<20; i++)
				promedio_der += pared_der[i];
			promedio_der = promedio_der/20;

			if(/*sensor_izq < 250 && sensor_der < 250*/ promedio_izq < 200 && promedio_der < 200 && cont2 > 3)
				SUPERESTADO = Laberinto;
			else
				cont2 += Periodo;
		}

		if(SUPERESTADO == Laberinto)
		{
			leer_pared();
			loop_PID_lab();
		...
		else // if(SUPERESTADO == Rastreador)
		{
