	...
	else if(GIRO == REC && secuencia_actual != '7')	// GIRO == REC y hay más de una línea
	{
		if(abs((float)aux_act[1] - (S-1)/2) < abs((float)aux_act[0] - (S-1)/2))	// aux_act[1] está más cerca del centro que aux_act[0]
		{
			...
			for(i=aux_act[1]+2; i<S; i++)
				LINEAresult[i] = '0';
			giro = DER;	// Evitar que se pierda la línea si es giro cerrado a derecha y no bifurcación
						// Se establece a derechas porque es el único camino que sabemos que existe
		}
...
void enviar_secuencias(char sensores[S])
{
	...
	for(i=0; i<S; i++)
	{
		if((sensores[i] == '0' && mensaje[i] == '_') || (sensores[i] == '1' && mensaje[i] == '0'))
			sum++;
		else
		{
			if(sensores[i] == '0')
	...
	{
		if(mensaje_anterior[0] != 'X')
		{
			ins_mensaje(mensaje_anterior);
			ins_mensaje("\t");
			sprintf(num, "%u", rep);
			ins_mensaje(num);
			ins_mensaje("\n\r");
...
void regular_PID(void)
{
	...
	AD1CR &= ~((1<<24)|(1<<3));
	adc = (AD1DR>>6)&0x03FF;
	Kd = (float) adc*100/1023;

	sprintf(cadena, "Kp: %g", Kp);   
	LCDPutStr(cadena, 100, 20, SMALL, WHITE, BLACK);
	sprintf(cadena, "Ki: %g", Ki);   
	LCDPutStr(cadena, 80, 20, SMALL, WHITE, BLACK);
	sprintf(cadena, "Kd: %g", Kd);   
	LCDPutStr(cadena, 60, 20, SMALL, WHITE, BLACK);
}
