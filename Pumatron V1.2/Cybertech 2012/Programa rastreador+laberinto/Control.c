...
{
	if(ADCresult1[ch] > 23)
		distancia[ch] = 25*((3000.0 / (ADCresult1[ch] - 23) + 0.3)); // Fórmula de conversión de tensión a mm.
	else
		distancia[ch] = 800;
...
{
	distancia[ch] = 16.8*((2914.0 / (ADCresult1[ch] + 35) - 0.5)); // Fórmula de conversión de tensión a mm.
...
//
//	sprintf(cadena, "%3c", SUPERESTADO);   
//	LCDPutStr(cadena, 10, 10, SMALL, WHITE, BLACK);
}
